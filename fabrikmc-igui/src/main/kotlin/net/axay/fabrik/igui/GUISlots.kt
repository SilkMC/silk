...
interface InventorySlotCompound<out T : ForInventory> {
    ...
    fun withInvType(invType: IGUIType<T>): Collection<InventorySlot>

    fun realSlotsWithInvType(invType: IGUIType<T>) =
...
open class SingleInventorySlot<T : ForInventory> internal constructor(
    ...
    override fun withInvType(invType: IGUIType<T>) = slotAsList
...
class InventorySlotRange<out T : ForInventory> internal constructor(
    ...
    override fun withInvType(invType: IGUIType<T>) = LinkedHashSet<InventorySlot>().apply {
        when (type) {
...
class InventoryRowSlots<T : ForInventory> internal constructor(
    ...
    override fun withInvType(invType: IGUIType<T>) = HashSet<InventorySlot>().apply {
        for (slotInRow in 1..invType.dimensions.width)
            this += InventorySlot(row, slotInRow)
    }
...
class InventoryAllSlots<T : ForInventory> : InventorySlotCompound<T> {
    override fun withInvType(invType: IGUIType<T>) = invType.dimensions.invSlots
}
